...
class TrexTrafficGenerator():
    ...
    def connect(self):
#        self._conn = STLClient(server=self._params["server"])
	...
	core_mask_array = []
        ...
        if self._params["warmup"] > 0:
            self._conn.start(ports=traffic_ports, mult=mult, duration=self._params["warmup"], core_mask=[0x1, 0x2, 0x3, 0x4])
        ...
        self._conn.start(ports=traffic_ports, mult=mult, duration=self._params["duration"], core_mask=[0x1, 0x2, 0x3, 0x4])
