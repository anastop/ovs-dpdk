...
$OVS_DIR/vswitchd/ovs-vswitchd unix:$DB_SOCK \
	...
	--detach  \
	--log-file=/usr/local/var/log/openvswitch/ovs-vswitchd.log
$OVS_DIR/utilities/ovs-vsctl set Open_vSwitch . other_config:pmd-cpu-mask=0x1E000000001E00000 #4C48 -- uses CPUs 21,22,23,24,61,62,63,64
$OVS_DIR/utilities/ovs-vsctl set Open_vSwitch . other_config:max-idle=30000
...
$OVS_DIR/utilities/ovs-vsctl add-br br1 -- set bridge br1 datapath_type=netdev
$OVS_DIR/utilities/ovs-vsctl add-br br2 -- set bridge br2 datapath_type=netdev
$OVS_DIR/utilities/ovs-vsctl add-br br3 -- set bridge br3 datapath_type=netdev
...
ifconfig br1 0 up
$OVS_DIR/utilities/ovs-vsctl add-port br1 dpdk1 -- set Interface dpdk1 type=dpdk options:dpdk-devargs=0000:af:00.1 other_config:pmd-rxq-affinity="0:22"
sleep 3

ifconfig br2 0 up
$OVS_DIR/utilities/ovs-vsctl add-port br2 dpdk2 -- set Interface dpdk2 type=dpdk options:dpdk-devargs=0000:b1:00.0 other_config:pmd-rxq-affinity="0:23"
sleep 3

ifconfig br3 0 up
$OVS_DIR/utilities/ovs-vsctl add-port br3 dpdk3 -- set Interface dpdk3 type=dpdk options:dpdk-devargs=0000:b1:00.1 other_config:pmd-rxq-affinity="0:24"
sleep 3
...
$OVS_DIR/utilities/ovs-vsctl add-port br1 vhost-user1 -- set Interface vhost-user1 type=dpdkvhostuser other_config:pmd-rxq-affinity="0:62"
$OVS_DIR/utilities/ovs-vsctl add-port br2 vhost-user2 -- set Interface vhost-user2 type=dpdkvhostuser other_config:pmd-rxq-affinity="0:63"
$OVS_DIR/utilities/ovs-vsctl add-port br3 vhost-user3 -- set Interface vhost-user3 type=dpdkvhostuser other_config:pmd-rxq-affinity="0:64"
...
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=1,dl_type=0x800,nw_dst=24.0.0.0/8,idle_timeout=0,action=output:5
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=2,dl_type=0x800,nw_dst=16.0.0.0/8,idle_timeout=0,action=output:6
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=3,dl_type=0x800,nw_dst=48.0.0.0/8,idle_timeout=0,action=output:7
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=4,dl_type=0x800,nw_dst=32.0.0.0/8,idle_timeout=0,action=output:8
# 
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=5,dl_type=0x800,nw_dst=16.0.0.0/8,idle_timeout=0,action=output:1
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=6,dl_type=0x800,nw_dst=24.0.0.0/8,idle_timeout=0,action=output:2
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=7,dl_type=0x800,nw_dst=32.0.0.0/8,idle_timeout=0,action=output:3
# $OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=8,dl_type=0x800,nw_dst=48.0.0.0/8,idle_timeout=0,action=output:4

$OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=1,dl_type=0x800,nw_dst=24.0.0.0/8,idle_timeout=0,action=output:2
$OVS_DIR/utilities/ovs-ofctl add-flow br1 in_port=1,dl_type=0x800,nw_dst=16.0.0.0/8,idle_timeout=0,action=output:2
$OVS_DIR/utilities/ovs-ofctl add-flow br2 in_port=1,dl_type=0x800,nw_dst=48.0.0.0/8,idle_timeout=0,action=output:2
$OVS_DIR/utilities/ovs-ofctl add-flow br3 in_port=1,dl_type=0x800,nw_dst=32.0.0.0/8,idle_timeout=0,action=output:2

$OVS_DIR/utilities/ovs-ofctl add-flow br0 in_port=2,dl_type=0x800,nw_dst=16.0.0.0/8,idle_timeout=0,action=output:1
$OVS_DIR/utilities/ovs-ofctl add-flow br1 in_port=2,dl_type=0x800,nw_dst=24.0.0.0/8,idle_timeout=0,action=output:1
$OVS_DIR/utilities/ovs-ofctl add-flow br2 in_port=2,dl_type=0x800,nw_dst=32.0.0.0/8,idle_timeout=0,action=output:1
$OVS_DIR/utilities/ovs-ofctl add-flow br3 in_port=2,dl_type=0x800,nw_dst=48.0.0.0/8,idle_timeout=0,action=output:1

$OVS_DIR/utilities/ovs-ofctl dump-flows br0
$OVS_DIR/utilities/ovs-ofctl dump-flows br1
$OVS_DIR/utilities/ovs-ofctl dump-flows br2
$OVS_DIR/utilities/ovs-ofctl dump-flows br3 
