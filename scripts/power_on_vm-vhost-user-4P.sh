...
#vm=/root/vm-images/VPP-vm.qcow2
vm=/root/ovs-dpdk/vm-images/ubuntu-16.04-testpmd.img
vm_name=VPP-VM1
vnc=1
sshport=2023
echo "ssh $sshport"
...
then
    ...
    echo "VM $vm started! VNC: $vnc, ssh port: $sshport"
    taskset -c 6-11  /root/ovs-dpdk/qemu-2.12.1/x86_64-softmmu/qemu-system-x86_64 -m 8G -smp 6,cores=6,threads=1,sockets=1 -cpu host -drive format=raw,file=$vm  -boot c -enable-kvm -name $vm_name \
-object memory-backend-file,id=mem,size=8G,mem-path=/dev/hugepages,share=on -numa node,memdev=mem -mem-prealloc \
-netdev user,id=nttsip,hostfwd=tcp::$sshport-:22 \
